...
@section('content')
    <div class="min-h-screen flex flex-col space-y-6">
        ...
        <div class="bg-gray-800 shadow-md rounded p-6 w-full">
            <form action="{{route('kategori.update',$category->id)}}" method="POST" class="grid grid-cols-1 md:grid-cols-1 gap-6">
                @csrf
                @method('PUT')

                <div class="col-span-1">
                    <label for="nama" class="block text-sm font-medium text-gray-300 mb-1">Nama Kategori</label>
                    <input type="text" id="nama" name="name" value="{{old('name',$category->name)}}"
